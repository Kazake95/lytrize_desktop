...
# or: sudo zypper install rpm-build  # openSUSE
#
# Output  : build/lytrize-<version>-<release>.<arch>.rpm

# Install
#sudo rpm -i build/lytrize-1.0-amd64.rpm
...
SPEC=packaging/rpm/lytrize.spec

# ── Read metadata from the spec file (single source of truth) ─────────────────
...
VENV_REL="opt/$APP/venv"                   # relative path inside staging
VENV="$STAGING/$VENV_REL"
FINAL_RPM="build/${APP}-${VERSION}-${RELEASE}.${ARCH}.rpm"

echo "======================================"
echo "  Lytrize .rpm builder"
    ...
    if ! command -v "$tool" &>/dev/null; then
        case "$tool" in
            rpmbuild)
                ...
                ;;
            *)
                echo "ERROR: '$tool' not found. Install it and try again."
                ;;
        esac
        exit 1
    fi
...
cp packaging/usr/local/bin/lytrize   "$STAGING/usr/local/bin/lytrize"
cp packaging/usr/share/applications/lytrize.desktop \
                                     "$STAGING/usr/share/applications/lytrize.desktop"

# Remove Python caches and stray editor copy files
find "$STAGING/opt/$APP" -type d -name '__pycache__' -exec rm -rf {} + 2>/dev/null || true
find "$STAGING/opt/$APP" -name '*.pyc' -o -name '*.pyo' -delete 2>/dev/null || true
...
INSTALL_VENV="/opt/$APP/venv"

find "$VENV/bin" -maxdepth 1 -type f | while read -r f; do
    if head -c 2 "$f" 2>/dev/null | grep -q '#!'; then
        sed -i "1s|#!${VENV_ABS}/bin/python[0-9.]*|#!${INSTALL_VENV}/bin/python3|g" "$f"
    fi
done
...
find "$VENV" -name '*.pyc' -o -name '*.pyo' -delete 2>/dev/null || true

# ── Set permissions in staging before rpmbuild picks them up ──────────────────
...
# Copy spec into the rpmbuild SPECS dir
...
# Move the RPM out of the nested RPMS/<arch>/ tree to the flat build/ dir
BUILT=$(find "$RPMBUILD_ROOT/RPMS" -name "*.rpm" | head -1)
if [ -z "$BUILT" ]; then
    echo "ERROR: rpmbuild finished but no .rpm file was found."
    exit 1
fi
