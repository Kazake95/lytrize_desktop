...
def _make_outlier_fig(df: pd.DataFrame, col: str, info: dict) -> go.Figure:
    ...
    _sample_note = (
        f"  ·  background sampled to {_OUTLIER_NORMAL_MAX:,} pts"
        if was_sampled else ""
    )
    fig.update_layout(
        title=f"Outliers — {col}  ({n_out} outlier{'s' if n_out != 1 else ''} detected{_sample_note})",
...
def run_outlier_upload(df: pd.DataFrame) -> None:  # Full interactive upload-page widget: detect → view → delete outliers.
    ...
    st.caption(
        ...
        "**The IQR multiplier determines the strictness of the outlier threshold by defining how far from the median"
        " (in multiples of the IQR) a data point must be to be flagged, commonly using (1.5) for mild outliers / (3.0) for extreme outliers.**  \n"
...
def run_outlier(df, x_cols=None, y_cols=None, palette=None, **kwargs):  # Legacy runner — kept for backwards compatibility with saved sessions.
    ...
    for col in num:
        ...
        fig.update_layout(
            title=f"Outliers — {col}  ({n_out} outlier{'s' if n_out != 1 else ''} detected{_s_note})",
            xaxis_title="Row Index (hover to see exact row number)",
            yaxis_title=col,
            **chart_layout(),
        )
