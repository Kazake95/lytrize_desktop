...
│   │   ├── postinst             Post-install hook (icon cache, venv re-link)
...
2. Copy backend and desktop source files
3. Create a Python virtual environment at `build/lytrize/opt/lytrize/venv`
...
7. Call `dpkg-deb` to produce `build/lytrize.deb`

Install and test the built package:

```bash
sudo dpkg -i build/lytrize.deb
lytrize
```
